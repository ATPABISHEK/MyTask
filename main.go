...
func postRequester(w http.ResponseWriter, r *http.Request) {
	requestBody, err := json.Marshal(map[string]string{
		"title":    "The ATP",
		"subtitle": "The world biggest company",
		"content":  "Business times anounced that ATP is the biggest company  in the world",
	})
	errorHandler(err)
	resp, err := http.Post("http://localhost:8080/article/", "application/json", bytes.NewBuffer(requestBody))
	errorHandler(err)
